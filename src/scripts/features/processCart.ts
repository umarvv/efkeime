export function processCart() {
    const productsInMenu = document.querySelectorAll<HTMLDivElement>('.product__infos')
    const productsInCart = document.querySelectorAll<HTMLDivElement>('.product__in__cart__overview')
    const productsToSave = []

    productsInCart.forEach((product) => {
        const productInCartInfos = product.querySelector<HTMLDivElement>('.product__in__cart__infos')
        if(!productInCartInfos) return
        ...
        productsInMenu.forEach((productMenu) => {
            if (productMenu.dataset.name === productNameInCart) {
                ...
                const productImg = productContainer.querySelector<HTMLImageElement>('.product__img')
                if(!productImg || !productContainer) return
